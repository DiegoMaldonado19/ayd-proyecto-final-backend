...
public class PlateChangeRequestController {
        ...
        @Operation(summary = "Solicitar cambio de placa", description = "Crea una nueva solicitud de cambio de placa vehicular. Clientes solo pueden crear solicitudes para sus propias suscripciones.")
        @ApiResponses(value = {
                        ...
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "No autenticado", content = @Content),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "Sin permisos - No puedes crear solicitud para suscripción de otro usuario", content = @Content),
        ...
        @PostMapping
        @PreAuthorize("hasAnyRole('Administrador', 'Operador Back Office') or (hasRole('Cliente') and @subscriptionSecurity.isOwner(#request.subscriptionId))")
        public ResponseEntity<ApiResponse<PlateChangeRequestResponse>> createPlateChangeRequest(
                ...
                return ResponseEntity.status(HttpStatus.CREATED)
                                .body(ApiResponse.success(response,
                                                "Solicitud de cambio de placa creada exitosamente"));
        }
        ...
        @Operation(summary = "Subir evidencia", description = "Sube un archivo de evidencia para una solicitud de cambio de placa. Clientes solo pueden subir evidencia a sus propias solicitudes.")
        @ApiResponses(value = {
                        ...
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "No autenticado", content = @Content),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "Sin permisos - No puedes subir evidencia a solicitud de otro usuario", content = @Content),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Solicitud no encontrada", content = @Content)
        })
        @PostMapping(value = "/{id}/evidence", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @PreAuthorize("hasAnyRole('Administrador', 'Operador Back Office') or (hasRole('Cliente') and @plateChangeSecurity.isOwner(#id))")
